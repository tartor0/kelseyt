      ...
      <div className={`fixed top-6 right-6 z-50 transition-all duration-500 transform ${showNotification ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0 pointer-events-none'}`}>
        <div className="bg-[#111118] border-l-4 border-[#1E6F9F] shadow-2xl px-6 py-4 max-w-md backdrop-blur-sm">
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setShowNotification(false)} className="ml-auto text-white/30 hover:text-white/60 transition-colors">
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[8px] md:text-xs tracking-wider text-white/40 mt-2 font-light uppercase">
              {services.map((service, index) => (
                <div key={index} className="flex items-center gap-x-3">
                  <Link
                    href={service.path}
                    className="flex items-center gap-1.5 hover:text-[#1E6F9F] transition-colors cursor-pointer group"
                  >
                    <span className="opacity-50 group-hover:opacity-100 transition-opacity text-current">
                      {service.icon}
                    </span>
                    {service.name}
                  </Link>
        ...
        <div className="flex-1 flex items-center justify-center pointer-events-auto px-4">
          <div className="max-w-4xl mx-auto w-full text-center">
            <div className="space-y-8">
              <div className="flex justify-center gap-2 text-4xl md:text-5xl text-[#1E6F9F]/40">
            ...
            <div className="mt-12 max-w-md mx-auto">
              <form onSubmit={handleSubmit} className="space-y-4">
                <input
                  ...
                  className="w-full bg-[#0A0A0F]/60 backdrop-blur-sm border border-white/10 px-5 py-4 text-sm text-white placeholder-white/30 focus:outline-none focus:border-[#1E6F9F] transition-colors font-light text-center"
                  required
                />
                <button
                  type="submit"
                  className="w-full bg-[#1E6F9F] hover:bg-[#16567d] text-white text-sm font-medium px-6 py-4 flex items-center justify-center gap-3 transition-all group relative overflow-hidden"
                >
                  <span className="relative z-10">Wake Me Up When It's Ready</span>
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="relative z-10 group-hover:rotate-12 transition-transform">
